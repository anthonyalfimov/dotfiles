# Config for Powerlevel10k that is inspired by Powerline with powerline-gitstatus
#
# Based on romkatv/powerlevel10k/config/p10k-classic.zsh
#
# Tip: One-liner to print a colormap of terminal colours:
#
...
() {
  ...
  # restarting zsh. Edit ~/.dotfiles/p10k/p10k.zsh and type `source ~/.dotfiles/p10k/p10k.zsh`.
  ...
  function my_git_formatter() {
    ...
    local     normal_fg='%247F'   # gray foreground
    local     accent_fg='%252F'   # lightgray foreground
    local      clean_fg='%2F'     # green foreground
    local   detached_fg='%99F'    # mediumpurple foreground
    local        tag_fg='%7F'     # lightbrown foreground
    local     staged_fg='%2F'     # green foreground
    local   modified_fg='%166F'   # mediumorange foreground
    local  untracked_fg='%214F'   # brightestorange foreground
    local conflicted_fg='%160F'   # brightred foreground
    local    stashed_fg='%31F'    # darkblue foreground
    ...
    if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
      ...
      local branch_fg
      # Set the branch name foreground colour based on whether it's "dirty" or "clean"
      if (( VCS_STATUS_HAS_STAGED || VCS_STATUS_HAS_UNSTAGED || VCS_STATUS_HAS_UNTRACKED || VCS_STATUS_HAS_CONFLICTE )); then
        branch_fg=${normal_fg}
      else
        branch_fg=${clean_fg}
      fi
    ...
    if [[ -n ${VCS_STATUS_REMOTE_BRANCH:#$VCS_STATUS_LOCAL_BRANCH} ]]; then
      res+="${normal_fg} : ${accent_fg}${(V)VCS_STATUS_REMOTE_BRANCH//\%/%%}"
    fi
    ...
    # in this case.
    (( VCS_STATUS_HAS_UNSTAGED == -1 )) && res+=" ${modified_fg}─"
  ...
  # Custom prefix.
  # typeset -g POWERLEVEL9K_VCS_PREFIX='%247Fon '
